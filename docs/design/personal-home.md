...
Homepage only: 1200px shell; article reading widths stay unchanged. Existing Archivo, Source Serif, JetBrains Mono and theme tokens. Responsive headline 50–101px; section spacing 64–112px. Hero and CraftNote split into columns above 700px, stack below. Portrait arch becomes a square crop on small screens. Work buttons form a row on mobile. No fixed overlay navigation. Natural section heights; browser zoom enabled. Focus indicators and reduced-motion-aware existing post skeletons.
